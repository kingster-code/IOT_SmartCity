    ...
    public class ZoneBusiness : IZoneBusiness
    {
        ...
        public List<AtmMesDTO> GetAtmZoneLast3Months()
        {
            ...
            for (int i = 0; i < 1; i++)
            {
                var a = new AtmMesDTO
                {
                    Co = (aux.Where(s => s.Date.Year == data.Year && s.Date.Month == data.AddMonths(i).Month)
                             .Select(s => s.CO)
                             .Sum()) / (aux.Where(s => s.Date.Year == data.Year && s.Date.Month == data.AddMonths(i).Month).Count()),
                    Co2 = (aux.Where(s => s.Date.Year == data.Year && s.Date.Month == data.AddMonths(i).Month)
                              .Select(s => s.CO2)
                              .Sum()) / (aux.Where(s => s.Date.Year == data.Year && s.Date.Month == data.AddMonths(i).Month).Count()),
                    Hc = (aux.Where(s => s.Date.Year == data.Year && s.Date.Month == data.AddMonths(i).Month)
                             .Select(s => s.HC)
                             .Sum()) / (aux.Where(s => s.Date.Year == data.Year && s.Date.Month == data.AddMonths(i).Month).Count()),
                    MP = (aux.Where(s => s.Date.Year == data.Year && s.Date.Month == data.AddMonths(i).Month)
                             .Select(s => s.MP)
                             .Sum()) / (aux.Where(s => s.Date.Year == data.Year && s.Date.Month == data.AddMonths(i).Month).Count()),
                    NOx = (aux.Where(s => s.Date.Year == data.Year && s.Date.Month == data.AddMonths(i).Month)
                              .Select(s => s.NOx)
                              .Sum()) / (aux.Where(s => s.Date.Year == data.Year && s.Date.Month == data.AddMonths(i).Month).Count()),
                    Rcho = (aux.Where(s => s.Date.Year == data.Year && s.Date.Month == data.AddMonths(i).Month)
                               .Select(s => s.Rcho)
                               .Sum()) / (aux.Where(s => s.Date.Year == data.Year && s.Date.Month == data.AddMonths(i).Month).Count()),
                    SOx = (aux.Where(s => s.Date.Year == data.Year && s.Date.Month == data.AddMonths(i).Month)
                              .Select(s => s.SOx)
                              .Sum()) / (aux.Where(s => s.Date.Year == data.Year && s.Date.Month == data.AddMonths(i).Month).Count()),
                    Zone = "",
        ...
        /// <summary>
        /// Gets the number of zones for display in the Monitor.
        /// </summary>
        /// <returns></returns>
        ...
        /// <summary>
        /// Gets the number of roberie per zone for display in the Monitor.
        /// </summary>
        /// <returns></returns>
        ...
        /// <summary>
        /// Gets the number of sensors for display in the Monitor
        /// </summary>
        /// <returns></returns>
